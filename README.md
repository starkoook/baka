...
- 本地 ONNX 打标改为真正的像素预处理，不再把编码后的图片当 raw 喂给模型。
- 标注写入失败会明确报错，不再假装保存成功。
- 图库扫描不再把 Sharp 等待放进 SQL 事务里，避免数据库锁死。
- 在线画廊默认不再强制走 `127.0.0.1:7890` 代理。
- 工作流保存 / 打开、回收站还原、FFmpeg 跨平台路径等一批日常崩溃已修。
