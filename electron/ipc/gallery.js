...
function registerGalleryHandlers(mainWindow) {
  ...
  ipcMain.handle('gallery:removeRoot', async (_event, { rootId, deleteImages }) => {
    try {
      await ensureDb()
      if (deleteImages) {
        const imgs = queryAll('SELECT id, thumb_hash FROM images WHERE root_id = ?', [rootId])
  ...
  ipcMain.handle('gallery:scan', async (_event, folderPath) => {
    try {
      ...
      for (const root of roots) {
        ...
        totalRemoved += res.removedCount || 0
      }
      return { success: true, data: { newCount: totalNew, skipCount: totalSkip, errorCount: totalErr, removedCount: totalRemoved } }
    } catch (e) {
      return { success: false, error: e.message }
    }
  })
...
module.exports = { registerGalleryHandlers, initDb, ensureDb, queryAll, queryOne, runSql, saveDb, generateThumbnail, hashPath, readFileMetaFromPath, classifyDroppedPaths, importImageFiles, scanFolder, IMAGE_EXTENSIONS }
